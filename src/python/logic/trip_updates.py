...
def fetch_trip_update_data(app_settings, c, trip_update_data):
...
def update_signals(app_settings, trip_update_data):
    signals = {
        'INBOUND': {
            'value': 'off',
            'reason': None
    ...
    if trip_update_data['STOPS']['INBOUND_TRIGGER'] is not None and trip_update_data['STOPS']['INBOUND_TARGET'] is not None:
        ...
        if trip_update_data['STOPS']['INBOUND_TRIGGER']['TOTAL_SECONDS'] <= -1 * app_settings['inbound_trigger_seconds']:
            signals['INBOUND']['value'] = 'on'
            signals['INBOUND']['reason'] = 'Info for INBOUND_TARGET & INBOUND_TRIGGER, INBOUND_TRIGGER still to depart for > > {} s'.format(app_settings['inbound_trigger_seconds'])
    ...
    if trip_update_data['STOPS']['OUTBOUND_TRIGGER'] is not None and trip_update_data['STOPS']['OUTBOUND_TARGET'] is not None:
        ...
        if trip_update_data['STOPS']['OUTBOUND_TRIGGER']['TOTAL_SECONDS'] <= -1 * app_settings['outbound_trigger_seconds']:
            signals['OUTBOUND']['value'] = 'on'
            signals['OUTBOUND']['reason'] = 'Info for OUTBOUND_TARGET & OUTBOUND_TRIGGER, OUTBOUND_TRIGGER still to depart for > {} s'.format(app_settings['outbound_trigger_seconds'])
